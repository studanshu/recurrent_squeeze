...
def save_checkpoint(state, is_best, filename='models/recurrentsqueezenet.tar'):
    ...
    if is_best:
        shutil.copyfile(filename, 'models/model_best_recurrent.tar')
...
if load_model:
    state=torch.load("models/model_best_recurrent.tar")
...
#     if epoch < 299: continue
    ...
    if epoch % save_fig_step == 0 and epoch > 0:
        ...
        plt.legend()
        plt.savefig('Recurrent_Attention_loss.jpg')
        plt.clf()
        #Accuracies.
        ...
        plt.legend()
        plt.savefig('Recurrent_Attention_accuracy.jpg')
        plt.clf()
